...
export const bulkImportMembers = createServerFn({ method: "POST" })
  ...
  .validator((input: { rows: BulkMemberRow[] }) => {
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const row of data.rows) {
      ...
      if (!email) {
        result.errors.push({ email: "(blank)", error: "Missing email" });
        continue;
      }
      ...
      if (row.member_number != null && row.member_number !== "")
        update.member_number = String(row.member_number).trim();
      if (row.full_name != null && row.full_name !== "")
        update.full_name = String(row.full_name).trim();
      if (row.phone != null && row.phone !== "") update.phone = String(row.phone).trim();
      if (row.joined_at != null && row.joined_at !== "")
        update.joined_at = new Date(row.joined_at).toISOString();
      if (row.opening_balance != null && !Number.isNaN(Number(row.opening_balance)))
        update.opening_balance = Number(row.opening_balance);
    ...
    await supabaseAdmin.from("audit_log").insert({
      ...
      meta: {
        ...
        counts: {
          ...
          not_found: result.notFound.length,
