...
interface Props { mode: "member" | "admin"; userId?: string; title?: string }
...
export function RepaymentTrendChart({ mode, userId, title }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-border/70 bg-card p-6 shadow-[var(--shadow-card)]">
      <h3 className="text-base font-semibold text-foreground">{title ?? (mode === "admin" ? "Repayment trend (all members)" : "Your loan repayments")}</h3>
      ...
      <div className="mt-4 h-64 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ left: -10, right: 8, top: 10, bottom: 0 }}>
            ...
            />
            <Line type="monotone" dataKey="total" stroke="oklch(0.55 0.18 25)" strokeWidth={2.5} dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
