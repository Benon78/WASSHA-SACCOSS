import { Check, Clock, FileText, ShieldCheck, Banknote, UserCheck, Building2, Sparkles } from "lucide-react";
    ...
    <div className="rounded-2xl border border-border/70 bg-card p-6 shadow-[var(--shadow-card)]">
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Currently with Finance Officer · est. 1 day remaining</p>
        </div>
        ...
        {stages.map((s, i) => {
          ...
          return (
            <li key={s.label} className="relative flex flex-col items-center text-center">
              ...
              <p className="mt-2 text-[11px] font-semibold leading-tight text-foreground">{s.label}</p>
              {i < stages.length - 1 && (
