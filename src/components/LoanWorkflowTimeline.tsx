        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {isRejected
              ...
              : `Currently at: ${STAGE_LABEL[currentStage] ?? currentStage}`}
          </p>
        </div>
        <span
        ...
        <ol className="relative flex min-w-max items-start gap-4 md:gap-2 lg:grid lg:grid-cols-9 lg:min-w-0">
          {STAGE_ORDER.map((stage, i) => {
            ...
            return (
              <li
                key={stage}
                className="flex w-24 shrink-0 flex-col items-center text-center lg:w-auto"
              >
                ...
                <p className="mt-2 text-[11px] font-semibold leading-tight text-foreground break-words">
                  {STAGE_LABEL[stage]}
                </p>
              </li>
            );
