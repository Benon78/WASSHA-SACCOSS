...
function NotificationsPage() {
  ...
  const markAll = useMutation({
    ...
    onSuccess: () =>
      user && queryClient.invalidateQueries({ queryKey: ["notifications", user.id] }),
  });
  ...
  const setFilter = (patch: Partial<{ type: NotifType; state: "all" | "unread" }>) => {
    nav({ search: { type, state, page: 1, ...patch }, replace: true });
  };
  const goPage = (p: number) => nav({ search: { type, state, page: p }, replace: true });
      ...
      <div className="container mx-auto max-w-3xl space-y-6 px-4 py-6">
        ...
        {query.isLoading ? (
          ...
          <EmptyState
            ...
            description={
              state === "unread" ? "You're all caught up." : "No notifications match these filters."
            }
          />
        ) : (
              ...
              const Body = (
                <div className="flex items-start gap-3 p-4">
                  ...
                  <button
                    ...
                    aria-label={
                      n.read ? "Mark notification as unread" : "Mark notification as read"
                    }
              ...
              return (
                <li key={n.id}>
                  {n.link ? (
                    <Link
                      to={n.link}
                      onClick={() => !n.read && toggleRead.mutate(n)}
                      className={cls}
                    >
                      {Body}
                    </Link>
