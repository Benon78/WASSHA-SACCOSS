...
export const Route = createFileRoute("/_app/superadmin/")({
  head: () => ({
    meta: [
      { title: "Super Admin Dashboard — WASSHA SACCOS" },
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard
            ...
            tone={data.failedLogins24h > 10 ? "warning" : "primary"}
          />
          <StatCard
            icon={Activity}
            label="Audit events"
            value={data.auditEvents24h.toLocaleString()}
            tone="primary"
          />
          <StatCard
            icon={ScanLine}
            label="Completed loans"
            value={data.completedLoans.toLocaleString()}
            tone="success"
          />
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border/60 bg-background/60 p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Roles &amp; Permissions, Branches, Loan Policies, Security Center, Audit Center, System
          Settings, AI Configuration, Backups, and Monitoring modules land in the next rollout
          groups.
        </p>
      </div>
    </div>
