...
export const Route = createFileRoute("/_app/loans/simulator")({
  head: () => pageHead({
    ...
    description: "Estimate monthly repayments, total interest, processing fees and late-penalty exposure before applying.",
    noIndex: true,
  }),
...
function SimulatorPage() {
  ...
  const result = useMemo(() => {
    ...
    const rate =
      type === "emergency" ? Number(policy.emergency_rate) :
      type === "chapchap"  ? Number(policy.chapchap_rate)  :
    ...
    return { rate, feeRate, penaltyRate, monthly, totalPaid, totalInterest, fee, net, penalty, missed };
      ...
      <div className="container mx-auto max-w-4xl space-y-6 px-4 py-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Preview repayments before you apply. Uses the current active loan policy — no application is created.
          </p>
        </div>
        ...
        <section className="grid gap-4 rounded-2xl border border-border/70 bg-card p-6 shadow-[var(--shadow-card)] md:grid-cols-4">
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {(["development", "chapchap", "emergency"] as LoanType[]).map((k) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => setType(k)}
                  className={type === k ? "bg-[image:var(--gradient-primary)] text-primary-foreground" : ""}
                >
                  {LOAN_TYPE_LABEL[k]}
                </Button>
              ))}
          ...
          <div className="md:col-span-2">
            <Label className="flex items-center gap-1"><AlertTriangle className="h-3.5 w-3.5 text-warning" /> Missed instalments (for penalty preview)</Label>
            <Input type="number" min="0" value={missedMonths} onChange={(e) => setMissedMonths(e.target.value)} />
          </div>
        </section>
        ...
        {result && (
          <section className="grid gap-4 md:grid-cols-2">
            <Card label="Monthly instalment" value={fmtTZS(Math.round(result.monthly))} icon={<TrendingUp className="h-4 w-4 text-primary" />} />
            <Card label={`Total repayable (${term} mo)`} value={fmtTZS(Math.round(result.totalPaid))} />
            <Card label="Total interest" value={fmtTZS(Math.round(result.totalInterest))} />
            <Card label={`Processing fee (${result.feeRate}%)`} value={fmtTZS(Math.round(result.fee))} sub={`Net disbursed: ${fmtTZS(Math.round(result.net))}`} />
            <Card
              label={`Late-payment penalty (${result.penaltyRate}% / month)`}
              value={fmtTZS(Math.round(result.penalty))}
              sub={result.missed > 0 ? `Applied for ${result.missed} missed instalment(s)` : "No missed instalments"}
            />
            <Card label="Annual rate" value={`${result.rate}%`} sub={`Effective monthly ≈ ${(result.rate / 12).toFixed(2)}%`} />
          </section>
        )}

        <p className="text-xs text-muted-foreground">
          Estimates only. Final terms are confirmed at approval and are subject to the policy version effective on your disbursement date.
        </p>
      </div>
    </div>
  );
