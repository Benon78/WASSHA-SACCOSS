...
function LoansList() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("loans").select("*").eq("member_id", user.id)
      ...
      <div className="container mx-auto space-y-6 px-4 py-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">All your loan applications and their progress.</p>
          </div>
          <Button asChild className="bg-[image:var(--gradient-primary)] text-primary-foreground shadow-[var(--shadow-elegant)] hover:opacity-95">
            <Link to="/loans/apply"><Plus className="mr-2 h-4 w-4" /> New application</Link>
          </Button>
        </div>
        ...
        <div className="rounded-2xl border border-border/70 bg-card shadow-[var(--shadow-card)]">
          {loans.length === 0 ? (
            <div className="flex flex-col items-center gap-3 p-12 text-center">
              ...
              <Button asChild><Link to="/loans/apply">Apply now</Link></Button>
            </div>
          ) : (
            <ul className="divide-y divide-border/70">
              {loans.map((l) => (
                <li key={l.id}>
                  <Link to="/loans/$loanId" params={{ loanId: l.id }} className="flex items-center justify-between p-5 transition hover:bg-muted/50">
                    <div>
                      <p className="text-sm font-bold">{l.loan_number}</p>
                      <p className="text-xs text-muted-foreground">{l.purpose} · {fmtDate(l.created_at)}</p>
                    </div>
