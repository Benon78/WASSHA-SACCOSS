...
} from "@/lib/bulk-members.functions";
      ...
      <div className="container mx-auto max-w-4xl space-y-6 px-4 py-6">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold">
            <Upload className="h-6 w-6 text-primary" /> Bulk member import
          </h1>
          <p className="text-sm text-muted-foreground">
            Upload a CSV to update existing member profiles. Members are matched by email — new
            accounts must still self-register or use the invite flow.
          </p>
        </div>
        ...
        <section className="rounded-2xl border border-border/70 bg-card p-6 shadow-[var(--shadow-card)]">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            ...
            <code className="rounded bg-muted px-1">member_number</code>,{" "}
            <code className="rounded bg-muted px-1">full_name</code>,{" "}
            <code className="rounded bg-muted px-1">phone</code>,{" "}
            <code className="rounded bg-muted px-1">joined_at</code> (YYYY-MM-DD),{" "}
            <code className="rounded bg-muted px-1">opening_balance</code>. Max 1000 rows per file.
          </p>
          <pre className="mt-3 overflow-x-auto rounded-lg border border-border/50 bg-muted/40 p-3 text-xs">
            {SAMPLE}
          </pre>
          <a
          ...
          {preview.length > 0 && (
            <>
              <p className="mt-4 text-sm text-muted-foreground">
                Parsed <strong>{preview.length}</strong> row(s) from{" "}
                <span className="font-mono">{file?.name}</span>. Preview (first 5):
              </p>
          ...
          <section className="rounded-2xl border border-border/70 bg-card p-6 shadow-[var(--shadow-card)]">
            ...
            <div className="mt-3 grid gap-2 text-sm">
              <div className="flex items-center gap-2 text-success">
                <CheckCircle2 className="h-4 w-4" /> {result.updated} profile(s) updated
              </div>
