...
export const Route = createFileRoute("/_app/superadmin/security")({
  head: () => ({
    meta: [{ title: "Security Center — Super Admin" }, { name: "robots", content: "noindex" }],
  }),
...
function SecurityPage() {
  ...
  if (overview.isLoading || sessions.isLoading)
    return <PageLoader label="Loading security center…" />;
  if (overview.error || !overview.data)
    return <ErrorState onRetry={overview.refetch} title="Failed to load security overview" />;
  if (sessions.error || !sessions.data)
    return <ErrorState onRetry={sessions.refetch} title="Failed to load sessions" />;
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Live view of authentication activity, sessions and suspicious IPs. All destructive actions
          require password re-verification and are audited.
        </p>
      </header>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Kpi
          icon={ShieldAlert}
          label="Accounts locked (7d)"
          value={o.lockedAccounts7d}
          tone={o.lockedAccounts7d > 0 ? "warn" : "ok"}
        />
                  ...
                  <td>
                    <ConfirmWithPassword
                      ...
                      onConfirmed={async (pw) => {
                        await termMut.mutateAsync({ sessionId: s.id, password: pw });
                      }}
                    />
                  </td>
                </tr>
